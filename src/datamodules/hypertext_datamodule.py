...
class HYPERTEXT_datamodule(LightningDataModule):
    def __init__(self,
        data_dir: str = "data/",
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        data = pd.read_csv(os.path.join(self.data_dir, 'hypertext.csv'), sep=',', header=None, names=['source', 'target', 'time'])
        data['time'] = data['time'] // 3600 # time interval in hour
        data = data.drop_duplicates(subset=['source', 'target', 'time'], keep='first')
        data = data.reset_index()
        data['source'] = data['source'] - 1 # node id starts from 0
        data['target'] = data['target'] - 1
        ## 0-8112 is training data
        train_df, val_df = data[data.time < 52], data[data.time >= 52]
        
        if self.snapshot:
            for i in range(10):
                train_df.loc[(int(i*0.1*52) <= train_df.time) & (train_df.time < int((i+1)*0.1*52)), 'time'] = i
