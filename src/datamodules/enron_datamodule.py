...
class ERON_datamodule(LightningDataModule):
    def __init__(self,
        data_dir: str = "data/",
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        data = pd.read_csv(os.path.join(self.data_dir, 'enron.csv'), sep=' ', header=None, names=['source', 'target', 'weight', 'time'])
        ...
        train_df, val_df = data[data.time < 919], data[data.time >= 919]
        
        if self.snapshot:
            for i in range(10):
                train_df.loc[(int(i*0.1*918) <= train_df.time) & (train_df.time < int((i+1)*0.1*918)), 'time'] = i
