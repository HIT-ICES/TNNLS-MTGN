...
class RT_datamodule(LightningDataModule):
    def __init__(self,
        data_dir: str = "data/",
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        data = pd.read_csv(os.path.join(self.data_dir, 'rt-pol.csv'), sep=',', header=None, names=['source', 'target', 'time'])
        ...
        train_df, val_df = data[data.time <= 948], data[data.time > 948]
        
        if self.snapshot:
            for i in range(10):
                train_df.loc[(int(i*0.1*948) <= train_df.time) & (train_df.time < int((i+1)*0.1*948)), 'time'] = i
